...
import { GitBranch, Activity, Search, Loader2 } from "lucide-react";
import { useState, useEffect, Suspense } from "react";
import { useSearchParams } from "next/navigation";

function SelfReferenceContent() {
  const searchParams = useSearchParams();
  const [inputValue, setInputValue] = useState("ن و ر");
  ...
  useEffect(() => {
    ...
    handleSearch(q ?? "ن و ر");
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // Group by surah, sort by count descending, take top 6
    ...
    <div className="space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-zinc-400">
          استكشاف كيف يفسر القرآن نفسه بالقرآن من خلال شبكة المعاني والتباينات.
        </p>
      </header>
      ...
      <div className="space-y-3">
        ...
        >
          <input
            ...
            placeholder="أدخل مفهوماً أو جذراً (مثل: ن و ر، ر ح م، ق ل ب)"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
              ...
              <div className="text-4xl font-amiri text-teal-400 mb-4 text-center">
                {query}
              </div>
              <p className="text-sm text-zinc-400 leading-relaxed text-center">
                تم رصد{" "}
                <span className="text-teal-400 font-medium">{totalResults}</span>{" "}
                آية عبر{" "}
                <span className="text-teal-400 font-medium">{totalSuras}</span>{" "}
                سورة
                {totalResults > 100 && (
                    ...
                    return (
                      <div key={sura} className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-teal-400">{verses.length}</span>
                        </div>
            ...
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/30 overflow-hidden">
              <div className="p-4 bg-zinc-900 border-b border-zinc-800 flex items-center gap-3">
                ...
                <h3 className="font-bold text-zinc-100">
                  خريطة الإحالات الدلالية
                </h3>
                <span className="text-xs text-zinc-500 mr-auto">
                  أبرز {topSuras.length} سور حسب التكرار
                </span>
              </div>
                ...
                {topSuras.map(([sura, verses]) => (
                  <div key={sura} className="space-y-2">
                    <div className="text-xs text-zinc-500 font-medium">
                      سورة {sura} —{" "}
                      <span className="text-teal-500">{verses.length} ورود</span>
                    </div>
                      ...
                      <div className="mt-2 flex items-center gap-3 text-xs text-zinc-500">
                        ...
                        {verses.length > 1 && (
                          <span className="text-zinc-600">
                            + {verses.length - 1} آية أخرى في هذه السورة
                          </span>
                        )}
                      </div>
                    </div>
